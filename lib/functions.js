...
export function calulateAverageChatDuration(data){
    ...
    Object.keys(data.records).forEach(record => {
        if (data.records[record].agents_chatting_duration) {
            count++
            avgResponseTime = avgResponseTime + data.records[record].agents_chatting_duration;
        }
    });
...
export function convertKeyToTeamName(key){
