...
export async function GET(request){
    ...
    try {
        ...
        const response = await fetch(`${process.env.LIVECHAT_API_URL}/reports/chats/total_chats`, {
            method: 'POST',
            headers: new Headers({
        ...
        //Response to get comparison data
        if(filter > 1){
            ...
            const responseTwo = await fetch(`${process.env.LIVECHAT_API_URL}/reports/chats/total_chats`, {
                method: 'POST',
                headers: new Headers({
                    'Content-Type': 'application/json',
                    "Authorization": `Basic ${btoa(`${process.env.LIVECHAT_API_USERNAME}:${process.env.LIVECHAT_API_PASSWORD}`)}`
